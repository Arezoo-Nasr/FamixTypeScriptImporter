...
//    private typeChecker! : TypeChecker;
    ...
    private generateInvocations() { // todo
    ...
    getClassNameOfMethod(savedMethod: MethodDeclaration | ConstructorDeclaration | MethodSignature) { // check
    ...
    private allProjectCallExpressions() { // check
    ...
    private generateNamespacesClassesInterfaces(sourceFiles: SourceFile[]) { // check
    ...
    private makeFamixIndexFileAnchor(sourceElement: PropertySignature | PropertyDeclaration | FunctionDeclaration | VariableDeclaration | ParameterDeclaration | Identifier | SourceFile | ModuleDeclaration | ClassDeclaration | InterfaceDeclaration | MethodDeclaration | MethodSignature | ConstructorDeclaration, famixElement: Famix.SourcedEntity) { // check
    ...
    private makeFamixIndexFileAnchorOld(filePath: string, startPos: number, endPos: number, famixElement: Famix.SourcedEntity) { // check
        let fmxIndexFileAnchor = new Famix.IndexedFileAnchor(this.fmxRep);
    ...
    //Arezoo
    private readNamespace(currentModules: ModuleDeclaration[] | SourceFile, file:SourceFile, parentScope: Famix.Namespace = null) { // check
...
//            console.info(`  classes: ${classesInFile.map(c => c.getName())}`);
    ...
    private addVariableElements(variablesInFile: VariableDeclaration[], fmxScope: Famix.Function | Famix.Namespace | Famix.Module) { // check
    ...
    private addFunctionElement(functionsInFile: FunctionDeclaration[], fmxNamespace: Famix.Namespace) { // check
    ...
    private addClassElements(classesInFile: ClassDeclaration[], file: SourceFile, fmxNamespace: Famix.Namespace) { // check
    ...
    private addInterfaceElements(interfacesInFile: InterfaceDeclaration[], fmxNamespace: Famix.Namespace) { // check
    ...
    //Arezoo
    private checkFamixNamespace(namespaceName: string, parentScope: Famix.Namespace = null): Famix.Namespace { // check
    ...
    private createFamixClass(cls: ClassDeclaration | InterfaceDeclaration, isInterface = false, isAbstract = false): Famix.Class { // check
    ...
    private createFamixGenerics(cls: ClassDeclaration | InterfaceDeclaration, isInterface = false): Famix.Class { // check
    ...
    private createFamixMethod(method: MethodDeclaration | ConstructorDeclaration | MethodSignature, isAbstract = false, isStatic = false): Famix.Method { // check
    ...
    private makeFamixLocalVariable(variable: VariableDeclaration) { // check
    ...
    private createFamixFunction(func: FunctionDeclaration): Famix.Function { // check
    ...
    private createFamixParameterType(tp: TypeParameterDeclaration) { // check
    ...
    private getUsableName(name: string): string { // check
    ...
    private getFamixType(typeName: string): Famix.Type { // check
...
function computeTSMethodSignature(methodText: string): string { // check
